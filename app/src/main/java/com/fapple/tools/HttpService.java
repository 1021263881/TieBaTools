...
public class HttpService
{
	...
	public String get_cookie(String Url, String cookie) throws mException 
	{
		//第一种方式
		ExecutorService executor = Executors.newCachedThreadPool();
		Task task = new Task("GET_cookie", Url, cookie);
		FutureTask<String> futureTask = new FutureTask<String>(task);
		executor.submit(futureTask);
		executor.shutdown();
		String re = "";
		try {
			...
			throw new mException("网络好像出了点问题喵~", "Get_cookie出错，错误信息:\n" + e.toString());
		} catch (ExecutionException e) {
			throw new mException("网络好像出了点问题喵~", "Get_cookie出错，错误信息:\n" + e.toString());
		}
		if (re == "" || re == null) {
	...
	private String Get_cookie(String Url, String cookie) throws IOException
	{
		//string转URL(utf-8)编码
		//产生UnsupportedEncodingException
		//Url = URLEncoder.encode(Url, "utf-8");
